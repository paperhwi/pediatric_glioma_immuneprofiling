...
const H3 = (text) => new Paragraph({
  ...
  children: [new TextRun({ text, font: FONT_TITLE, size: 22, bold: true, italics: true, color: "1A1A1A" })],
});
...
// Cite placeholder
const cite = (n) => new TextRun({ text: `[${n}]`, font: FONT, size: 22, superScript: false });
...
const titleRuns = [
  ...
  new TextRun({ text: "Brain-Tuned Deconvolution Reveals Microglia-versus-Macrophage Axis with Independent Prognostic Value",
                font: FONT_TITLE, size: 36, bold: true, color: "1A1A1A" }),
];
...
const doc = new Document({
  styles: {
    ...
    paragraphStyles: [
      ...
      { id: "Heading3", name: "Heading 3", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { size: 22, bold: true, italics: true, font: FONT_TITLE },
  ...
  sections: [{
    ...
    children: [
      ...
      blank(),
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 160 }, children: authors }),
      ...affil,
      blank(),
      ...
      P("[INSERT Figure 1: Study workflow — cohort assembly, deconvolution, ssGSEA, consensus clustering]", { italics: true, color: "555555" }),
      blank(),
      ...
      P("[INSERT Figure 2: Cross-method myeloid signal correlation heatmap with microglia and MDM clusters annotated]", { italics: true, color: "555555" }),
      ...
      P("[INSERT Figure 3: (A) Heatmap of features by ecotype; (B) PCA; (C) UMAP]", { italics: true, color: "555555" }),
      ...
      P("[INSERT Figure 4: Stacked barplots — (A) ecotype by cohort group; (B) ecotype by anatomical location]", { italics: true, color: "555555" }),
      ...
      P("[INSERT Figure 5: (A) Overall KM by ecotype; (B) Cox multivariable forest plot]", { italics: true, color: "555555" }),
      blank(),
      ...
      P("[INSERT Figure 6: Composite boxplots of 12 key signatures across ecotypes]", { italics: true, color: "555555" }),
      blank(),
      ...
      P("Conceptualization, methodology, data curation, formal analysis, writing — original draft, S.C.; visualization, S.C.; supervision, S.C. " +
        "The author has read and agreed to the published version of the manuscript.", { firstLine: false }),

      H1("Funding"),
      P("This research received no external funding.", { firstLine: false }),
      ...
      P("Not applicable.", { firstLine: false }),
      ...
      P("All data analyzed in this study are publicly available from the OpenPedCan v15 release (https://github.com/d3b-center/OpenPedCan-analysis). " +
        "Analysis scripts and intermediate output files are available at [GitHub/Zenodo repository to be added at submission].", { firstLine: false }),

      H1("Conflicts of Interest"),
      P("The author declares no conflict of interest.", { firstLine: false }),

      // ============ FIGURE LEGENDS ============
      new Paragraph({ children: [new PageBreak()] }),
      H1("Figure Legends"),

      H3("Figure 1. Study workflow."),
      P("Schematic overview of cohort assembly, expression matrix preparation, multi-method deconvolution, brain-tuned ssGSEA scoring, and consensus clustering. " +
        "Numbers within boxes indicate sample counts at each step.", { firstLine: false }),

      H3("Figure 2. Brain-tuned signatures resolve a microglia-versus-MDM axis."),
      P("Spearman correlation heatmap of macrophage and monocyte-related signals across four deconvolution methods (MCP-counter, quanTIseq, EPIC, xCell) and " +
        "five brain-tuned ssGSEA signatures (microglia from Klemm 2020, Bohlen 2020, and Antunes 2021 Mg-TAM; MDM/Mo-TAM from Klemm 2020 and Antunes 2021). " +
        "Boxed regions highlight tight internal correlation among microglia signatures (ρ > 0.98) and among MDM signatures (ρ = 0.94), with cross-cluster " +
        "correlation of ρ ≈ 0.40 demonstrating that the two cellular signals are resolvable. xCell macrophage tracks MDM (ρ = 0.86), illustrating the " +
        "peripheral-macrophage bias of conventional methods.", { firstLine: false }),

      H3("Figure 3. Consensus clustering identifies three reproducible immune ecotypes."),
      P("(A) Heatmap of z-scored feature matrix (quanTIseq immune cell fractions and 19 immune ssGSEA signatures × 349 samples) annotated by ecotype, " +
        "cohort group, and anatomical location. (B) Principal component analysis projection of the feature matrix, colored by ecotype. " +
        "(C) UMAP projection.", { firstLine: false }),

      H3("Figure 4. Ecotype distribution by histone class and anatomy."),
      P("(A) Stacked barplot of ecotype proportions across the four main cohort groups (Fisher's exact p = 1.3 × 10⁻³). " +
        "(B) Stacked barplot of ecotype proportions across anatomical location, restricted to clearly classified midline, hemispheric, and posterior fossa " +
        "tumors (Ambiguous and Other categories excluded; Fisher's exact p = 1.3 × 10⁻²).", { firstLine: false }),

      H3("Figure 5. Ecotype is an independent prognostic factor."),
      P("(A) Kaplan-Meier overall survival curves stratified by ecotype (n = 251 with OS data; log-rank p = 0.037). Risk table below shows numbers at risk at " +
        "each time point. (B) Forest plot of multivariable Cox proportional-hazards model adjusting for cohort group, age, and sex (n = 251).",
        { firstLine: false }),

      H3("Figure 6. Pathway activity across ecotypes."),
      P("Composite boxplot of 12 key brain-immune and immune-modulatory ssGSEA signatures across the three ecotypes. Kruskal-Wallis test was performed for " +
        "each signature; Benjamini-Hochberg-adjusted p-values are shown in each panel header. Note that cell-cycle/proliferation signature does not differ " +
        "across ecotypes (p = 0.94; not shown), confirming that ecotype distinctions reflect immune contexture rather than tumor proliferation.",
        { firstLine: false }),
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync("C:/Users/scott/Downloads/Open PBTA/output/Manuscript_Draft_v1.docx", buf);
  console.log("Manuscript_Draft_v1.docx written");
});
