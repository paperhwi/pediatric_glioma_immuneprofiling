...
| `feature_matrices/TPM_harmonization_quanTIseq_ssGSEA_z.tsv` | 349 x 34 | Standardized 10-feature quanTIseq plus 24-feature ssGSEA TPM sensitivity matrix |
...
| `signatures/` | gene sets | Brain immune-program GMT used by the submitted workflows |

`TPM34_summary.json` records the 34-feature TPM analysis dimensions and bootstrap settings.
`T5_analysis_set_reconciliation.tsv` documents the analysis-set counts, and
`T5_software_environment.tsv` records software/package versions, parameters, and random seeds.
`SHA256SUMS.tsv` covers every deposited file in this directory except itself.
